...
simpaths_output_dir <- readLines(file.path(results_path, "output_dir.txt"))

person_path <- file.path(simpaths_output_dir, "csv", "Person.csv")
bu_path <- file.path(simpaths_output_dir, "csv", "BenefitUnit.csv")
...
merged_data[demAge >= 16 & demAge <= 64, employed := FALSE]
merged_data[labC4 == "EmployedOrSelfEmployed", employed := TRUE]

# Calculate non-negative equivalised disposable income (with subzero values set to zero)
merged_data[, nonneg_equiv_disp_inc := pmax(yDispEquivYear, 0)]

# Limit to working-age population
final_data <- merged_data[demAge >= 25 & demAge <= 64]
