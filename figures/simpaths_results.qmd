...
bld<-fread(here::here("data", "simpaths_output", "baseline", "summarised_output.csv"))

bld[strata == "population" & time == 2030, .(
  seed,
  cummean = dplyr::cummean(mean_mcscase35)
)] |>
  ggpubr::ggline("seed", "cummean")
...
data <- raw_data[, .(
  runs = .N,
  mhcase = median(mean_mcscase45),
  mhcase_l = quantile(mean_mcscase45, 0.025),
  mhcase_u = quantile(mean_mcscase45, 0.975),
  emp = median(emp_rate),
  ...
  # rii = median(log_rii, na.rm = TRUE),
  # rii_l = quantile(log_rii, 0.025, na.rm = TRUE),
  # rii_u = quantile(log_rii, 0.975, na.rm = TRUE)
...
data[strata == "population", .(scenario, mhcase)]
...
comparison[strata == "demMaleFlag"] |>
  _[scenario.x != "baseline"] |>
  _[, .(scenario.x, demMaleFlag, Change = mean_mhcase.y - mean_mhcase.x)] |>
...
comparison[strata == "eduHighestC4"] |>
  _[scenario.x != "baseline"] |>
  _[, .(scenario.x, eduHighestC4, Change = mean_mhcase.y - mean_mhcase.x)] |>
