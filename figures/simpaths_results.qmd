...
raw_data$scenario <- as.character(raw_data)
...
```{r}
comparison[strata == "hh_structure"] |>
  _[scenario.x != "baseline"] |>
  _[, .(scenario.x, hh_structure, Change = mean_mhcase.y - mean_mhcase.x)] |>
  gghistogram(x = "Change", bins = 30, facet.by = c("scenario.x", "hh_structure")) +
  geom_vline(xintercept = 0)
```
